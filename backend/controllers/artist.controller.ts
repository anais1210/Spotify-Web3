...
export class ArtistController {
  ...
  async getArtistByAddress(req: express.Request, res: express.Response) {
    const address = req.params.address;
    const result = await ArtistService.getInstance().getArtistByAddress(
      address
    );
    if (result === null) {
      res.status(404).end();
  ...
  buildRouter(): express.Router {
    ...
    router.get("/address/:address", this.getArtistByAddress.bind(this));
