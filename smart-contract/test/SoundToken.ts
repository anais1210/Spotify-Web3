...
describe("SoundToken", function () {
  async function deployNFTuneContract() {
    ...
    const soundToken = await upgrades.deployProxy(SoundToken, [staff.target], {
      initializer: "initialize",
    });
  ...
  it("should mint nft", async function () {
    ...
    await soundToken.connect(artist).mint(artist.address, 100000);
    expect(await soundToken.balanceOf(artist.address)).to.equal(100000);
  });
  ...
  it("should burn token", async function () {
    const { soundToken, admin, artist } = await loadFixture(
      deployNFTuneContract
    );
    await soundToken.connect(artist).mint(admin.address, 100000);
